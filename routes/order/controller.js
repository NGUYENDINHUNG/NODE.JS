...
const { asyncForEach } = require("../../utitl");
...
module.exports = {
  ...
  getDetail: async (req, res, next) => {
    try {
      ...
      if (found) {
        return res.sendStatus(200).json({ code: 200, payload: found });
      }
  ...
  create: async function (req, res, next) {
    try {
      ...
      const errors = [];
      if (!getCustomer) errors.push("Khách hàng không tồn tại");
      if (!getEmployee) errors.push("Nhân viên không tồn tại");
